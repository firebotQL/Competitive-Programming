...
const findMatchingFolder = (
  ...
  if (currentNode.type === "dir" && currentNode.size >= freeUp) {
  ...
  for (const childNode of currentNode.children) {
    result = Math.min(
      result,
      findMatchingFolder(childNode, freeUp, " " + prefix)
    );
  }
  return result;
};
...
let freeUp = 30000000 - (70000000 - root.size);
console.log(freeUp);

console.log(findMatchingFolder(root, freeUp, "- "));
